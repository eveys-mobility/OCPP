...
This repository is in **Phase 2 — Full OCPP 1.6 Core** (in progress). Phase 1 is closed: the WS server, the seven Phase-1 handlers, the Postgres schema, and the local docker-compose stack all run. Phase 2 has landed the v1 protos, the gRPC server, all seven RPC bodies (same-pod routing) — `RemoteStart`, `RemoteStop`, `Reset`, `ChangeConfiguration`, `TriggerMessage`, `UnlockConnector`, `GetChargerStatus` — the Redis online registry, and the `MeterValues` → Kafka path. Still in flight: the cross-pod command bus (E2-10), Kafka emit on the other handlers (E2-8), and the rest of OCPP 1.6 Core actions (E2-1). See [`docs/02-tasks.md`](./docs/02-tasks.md).
