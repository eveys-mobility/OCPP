...
| TC_073 | Update Charge Point Password | 🟡 | [`api/credentials.py`](../src/eveys_ocpp/api/credentials.py) | [test_credentials.py](../tests/unit/api/test_credentials.py) | Operator-facing REST: `PUT/DELETE /api/v1/charge-points/{cp_id}/credentials`. Bcrypt hash computed server-side; plaintext never touches SQL or logs. Emits a `cp.credential_rotated` Kafka envelope (action + actor only) for SIEM audit. (#196) |
...
| TC_080 | Secure Firmware Update | 🟡 | [`transport/grpc_server.py`](../src/eveys_ocpp/transport/grpc_server.py) (`SignedUpdateFirmware`) + [`handlers/v16/signed_firmware_status_notification.py`](../src/eveys_ocpp/handlers/v16/signed_firmware_status_notification.py) | [test_grpc_server.py](../tests/unit/transport/test_grpc_server.py) (`test_signed_update_firmware_*`), [test_signed_firmware_status_notification.py](../tests/unit/handlers/v16/test_signed_firmware_status_notification.py), [test_commands.py](../tests/unit/api/test_commands.py) (`test_signed_update_firmware_*`) | OCPP 1.6 Security Whitepaper §4.4. Gateway is pure transport + boundary validation: parses signing-cert PEM at the boundary, builds the §4.4 firmware Dict (location / retrieveDateTime / signingCertificate / signature / optional installDateTime). Does NOT verify signature against blob (charger's job) or chain of trust (deferred — would need a trusted-roots registry). Inbound `SignedFirmwareStatusNotification` updates `last_firmware_status` with the wider 14-value enum including `SignatureVerified`. |
| TC_081 | Secure Firmware Update — Invalid Signature | 🟡 | [`handlers/v16/signed_firmware_status_notification.py`](../src/eveys_ocpp/handlers/v16/signed_firmware_status_notification.py) | [test_signed_firmware_status_notification.py::test_persists_invalid_signature_status](../tests/unit/handlers/v16/test_signed_firmware_status_notification.py) | Charger's `InvalidSignature` value round-trips verbatim through the handler to `last_firmware_status` and the `eveys_ocpp_firmware_status_total{status="InvalidSignature"}` counter. The two security-paging values `InvalidSignature` and `InstallVerificationFailed` are pinned in tests so a regression that filtered or remapped them would fail loud. |
...
| TC_074b | Update Charge Point Certificate by request of Charge Point (`SignCertificate.req` inbound) | 🟡 (operator-queue) | Inbound: [`handlers/v16/sign_certificate.py`](../src/eveys_ocpp/handlers/v16/sign_certificate.py); tests in [test_sign_certificate.py](../tests/unit/handlers/v16/test_sign_certificate.py). Operator queue: REST endpoints under `/api/v1/charge-points/{cp_id}/pending-certificate-signings` (list / get / approve / reject) at [`api/pending_certificate_signings.py`](../src/eveys_ocpp/api/pending_certificate_signings.py); approve dispatches `CertificateSigned.req` to the charger. Operators sign offline against whatever CA they choose today; an automated CA service is the remaining deferred follow-up tracked in #187. |
