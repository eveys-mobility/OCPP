...
| TC_074b | Update Charge Point Certificate by request of Charge Point (`SignCertificate.req` inbound) | 🟡 (handler-only) | [`handlers/v16/sign_certificate.py`](../src/eveys_ocpp/handlers/v16/sign_certificate.py); tests in [test_sign_certificate.py](../tests/unit/handlers/v16/test_sign_certificate.py). Charger-submitted CSRs are persisted into `pending_certificate_signings` and emitted on the `cp.csr_submitted` Kafka topic for operator review; the actual signing pipeline (CA selection, approval flow, `CertificateSigned.req` callback) is the deferred follow-up tracked in #187. |
