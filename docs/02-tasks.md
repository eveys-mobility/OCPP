...
| E3-9 | Implement webhook delivery — HMAC-signed, retried, dedupable; per-event URL / enable toggles per `docs/integration/03-webhooks.md` | 🟡 in progress — **first slice merged**: `src/eveys_ocpp/webhooks/` package with HMAC-SHA-256 signer, Kafka-tailing dispatcher, exponential-backoff retry (1/5/30/120/600 s defaults), per-event toggles (`webhook_enable_*`) and URL overrides, per-attempt `X-Eveys-*` headers. 13 new Settings (one new `webhooks` category). Wires `cp.boot` / `cp.status_changed` / `tx.started` end-to-end. 25 unit tests across signer + dispatcher. **Follow-up PRs**: ⬜ remaining events (`cp.online`/`offline`, `cp.firmware_status_changed`, `cp.diagnostics_status_changed`, `tx.stopped`); ⬜ E2E test against the in-repo mock backend; ⬜ ADR-0027 documenting the design choices (polling vs Kafka-consumer, HMAC vs JWT, retry table). |
