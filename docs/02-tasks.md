...
| E2-11 | Idempotency cache for `BootNotification` and `StopTransaction` | 🟡 partial — `StopTransaction` is idempotent today via Postgres key `cp_id:transaction_id:meter_stop` (`stop_transaction()` repository returns `applied=False` on replay; covered by `test_stop_transaction_replay_is_idempotent`). `BootNotification` is row-level idempotent (upsert on `cp_id`) but downstream-event dedup waits on E2-8 wiring `cp.boot` to Kafka — once Boot publishes, replay must not double-emit |
