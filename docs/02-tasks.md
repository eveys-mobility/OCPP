...
| E2-7 | Kafka producer (`aiokafka`) | 🟡 partial — `KafkaEventProducer` (aiokafka) shipped with E2-1; `start()`/`stop()` lifecycle wired into `__main__`. All four E2-8 emitters wrap `publish()` in try/except (broker drop logs `<handler>.publish_failed`, never raised — chargers retry, gateway stays up). Still TODO under this task: reconnect-on-broker-drop tuning, idempotent-producer flag, and batching defaults (`linger_ms`, `acks=all` for durability) — needs an ADR on the durability/latency trade-off. |
...
| E2-13 | ClickHouse table schemas for `MeterValues`, `Heartbeats`, `StatusNotifications` (DDL + Alembic-equivalent migration tool) | Tables exist in dev cluster; schemas live in `proto/events/` aligned with Kafka envelopes |
| E2-14 | Kafka → ClickHouse ingestion (Kafka table engine, or sidecar consumer if engine constraints push us off it) | Events produced by E2-8 visible in ClickHouse within seconds; lag dashboards live |
